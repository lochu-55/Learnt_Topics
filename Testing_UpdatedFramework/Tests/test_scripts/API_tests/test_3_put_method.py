...
def response():
    global c
    met = Methods()
    old_conf = Objects(r"Core/Elements/API_Data/data.json")
    old_conf.read_data()
    new_conf = Objects(r"Core/Elements/API_Data/new_data.json")
    new_conf.read_data()
...
def test_put_request_json_schema_validation(response, log):
    try:
        with open(r"Core/Elements/API_Data/data.schema.json", 'r') as schema_file:
