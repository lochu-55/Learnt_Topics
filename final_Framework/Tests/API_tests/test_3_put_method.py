...
from Tests.API_tests import test_1_post_method as old_conf
import jsonschema
import json
...
def response():
    global c
    met = Methods()
    old_conf.obj.read_data()
    new_conf = Objects(r"Base/Elements/Data/new_data.json")
    new_conf.read_data()
    payload = new_conf.data[c]
    value = old_conf.obj.data[c]["id"]
    res = met.put(id=value, data=payload)
    yield res
    c += 1

@pytest.mark.api
def test_put_request_status_code(response):
    old_conf.log.logger.info("tested status code")
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"

@pytest.mark.api
def test_put_request_response_time(response):
    old_conf.log.logger.info("tested response time")
    assert response.elapsed.total_seconds() < 5, "Response time exceeds 1 second"

@pytest.mark.api
def test_put_request_content_type(response):
    old_conf.log.logger.info("tested context type")
    assert response.headers["Content-Type"].startswith("application/json"), "Unexpected Content-Type"

@pytest.mark.api
def test_put_request_body_size(response):
    old_conf.log.logger.info("tested body size")
    assert len(response.content) < 1024, "Response body size exceeds 1024 bytes"

@pytest.mark.api
def test_put_request_json_schema_validation(response):
    try:
        ...
        old_conf.log.logger.info("tested json schema")
